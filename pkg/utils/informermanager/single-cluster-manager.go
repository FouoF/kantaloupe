...
var gvrTypeMap = map[reflect.Type]schema.GroupVersionResource{
	reflect.TypeOf(&corev1.Node{}):      NodeGVR,
	reflect.TypeOf(&corev1.Pod{}):       PodGVR,
	reflect.TypeOf(&corev1.Namespace{}): NamespaceGVR,
}
...
func NewSingleClusterInformerManager(client kubernetes.Interface,
	defaultResync time.Duration, parentCh <-chan struct{},
...
func (s *singleClusterInformerManagerImpl) ForResource(
	resource schema.GroupVersionResource, handler cache.ResourceEventHandler,
) error {
	s.lock.Lock()
	defer s.lock.Unlock()
	...
	if err != nil {
		klog.ErrorS(err, "Failed to add handler for resource", "resource", resource.String())
		return err
	}
...
func (s *singleClusterInformerManagerImpl) IsHandlerExist(
	resource schema.GroupVersionResource, handler cache.ResourceEventHandler,
) bool {
	s.lock.RLock()
	defer s.lock.RUnlock()
...
func (s *singleClusterInformerManagerImpl) Lister(resource schema.GroupVersionResource) (interface{}, error) {
	...
	if resource == NamespaceGVR {
		return s.informerFactory.Core().V1().Namespaces().Lister(), nil
	}

	return resourceInformer.Lister(), nil
}

func (s *singleClusterInformerManagerImpl) appendHandler(
	resource schema.GroupVersionResource, handler cache.ResourceEventHandler,
) {
...
func (s *singleClusterInformerManagerImpl) WaitForCacheSyncWithTimeout(
