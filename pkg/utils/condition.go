...
func GetConditionByType(conditions []metav1.Condition, condtionType string) *metav1.Condition {
	for _, cnd := range conditions {
		if cnd.Type == condtionType {
			condition := NewCondition(cnd.Type, cnd.Reason, cnd.Message, cnd.Status)
			return &condition
		}
	}
