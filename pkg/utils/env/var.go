...
func (v Var) String() string {
	...
	return fmt.Sprintf("%s\t\t= [%s] %s \t\t%s", v.Name, v.DefaultValue, typeStr, v.Description)
}
...
func Register[T Parseable](name string, defaultValue T, description string) GenericVar[T] {
	...
	b, _ := json.Marshal(defaultValue) //nolint:errchkjson
...
func RegisterStringVar(name, defaultValue, description string) StringVar {
...
func RegisterVar(v Var) {
	...
	if old, ok := allVars[v.Name]; ok {
		...
		if old.Description != v.Description ||
			old.DefaultValue != v.DefaultValue ||
			old.Type != v.Type ||
			old.Deprecated != v.Deprecated ||
			old.Hidden != v.Hidden {
			klog.V(1).InfoS("The environment variable is already registered with different values",
				"name", v.Name, "old", old, "new", v)
		}
	} else {
		allVars[v.Name] = v
	}
...
func (v BoolVar) Lookup() (bool, bool) {
	...
	if err != nil {
		klog.V(1).InfoS("Invalid environment variable value", "name", v.Name, "value", result, "default", v.DefaultValue)
