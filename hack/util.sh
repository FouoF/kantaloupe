...
function util::verify_go_version {
    ...
    if [[ "${MIN_Go_VERSION}" != $(echo -e "${MIN_Go_VERSION}\n${go_version[2]}" | sort -s -t. -k 1,1 -k 2,2n -k 3,3n | head -n1) && "${go_version[2]}" != "devel" ]]; then
      ...
      echo "kcollie requires ${MIN_Go_VERSION} or greater."
...
function util::install_environment_check {
    ...
    echo "Sorry, KCollie installation does not support $ARCH/$OS at the moment"
    exit 1
}
...
#  - $2: pod namespace, such as "kcollie-system"
...
function util::get_macos_ipaddress() {
	if [[ $(go env GOOS) = "darwin" ]]; then
		...
		echo ""
		echo " Detected that you are installing KCollie on macOS "
		echo ""
		echo "It needs a Macintosh IP address to bind KCollie API Server(port 5443),"
